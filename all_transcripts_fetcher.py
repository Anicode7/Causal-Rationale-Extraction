...
def sanitize_token(token, default='any'):
	"""Keep filename-safe chars for top_20 filenames."""
	if not token:
		return default
...
def _load_top20_from_dir(top20_dir):
	"""
	Load saved top_20_*.json files from top20_dir and return a dict of transcript_id -> transcript_obj.
	"""
	found = {}
	if not os.path.isdir(top20_dir):
		return found
	for path in glob.glob(os.path.join(top20_dir, "top_20_*.json")):
		try:
...
def categorize_query(query, llm, embeddings_db, data_path):
	...
	for i, data in enumerate(queries_list):
		...
		# ...existing code that writes top_20 files ...
	
	# Merge any transcripts already saved in data/top_20 so dialogue flow can use all of them
